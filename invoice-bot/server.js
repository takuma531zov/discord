...
const DISCORD_PUBLIC_KEY = process.env.DISCORD_PUBLIC_KEY || 'd9eecfefdeb293d1c6c7b07ce3fcd3c88a9cc90fdc031c026e095cd44aa7c39f';
...
app.post('/webhook', async (req, res) => {
  ...
  if (interaction.type === InteractionType.ModalSubmit) {
    if (interaction.data.custom_id === 'invoice-modal-simple') {
      ...
      if (basicInfo.length < 5 || amountInfo.length < 2) {
        return res.json({
          type: InteractionResponseType.ChannelMessageWithSource,
          data: {
            content: '❌ 入力形式が正しくありません。',
            flags: 64
          }
        });
      }
...
app.get('/', (req, res) => {
  res.json({ status: 'Discord Bot is running!', timestamp: new Date().toISOString() });
});
